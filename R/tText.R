...
#' @param df How to word the abbreviation for degrees of freedom. Defaults to "df".
...
tText <- function(x, df="df", sep="; ", dec="default", digits=c(3, 1, 3)){
  ...
  res <- paste(paste0("t = ", round(x$statistic, digits[1])),
    paste0(df, " = ", round(x$parameter, digits[2])),
    paste0("P = ", p), sep=sep)
